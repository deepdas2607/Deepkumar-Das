import{r,j as l}from"./threeUtils-CLSgAnG-.js";import{a8 as Oe}from"./interactions-CuK_Qb6V.js";const fe={maxVerticalRotationDeg:5,dragSensitivity:20,enlargeTransitionMs:300,segments:35},re=(f,z,A)=>Math.min(Math.max(f,z),A),De=f=>(f%360+360)%360,Ue=f=>((f+180)%360+360)%360-180,me=(f,z,A)=>{const Y=f.dataset[z]??f.getAttribute(`data-${z}`),X=Y==null?NaN:parseFloat(Y);return Number.isFinite(X)?X:A};function Ve(f,z){const A=Array.from({length:z},(n,a)=>-37+a*2),Y=[-4,-2,0,2,4],X=[-3,-1,1,3,5],D=A.flatMap((n,a)=>(a%2===0?Y:X).map(Q=>({x:n,y:Q,sizeX:2,sizeY:2}))),F=D.length;if(f.length===0)return D.map(n=>({...n,src:"",alt:"",description:""}));const _=f.map(n=>typeof n=="string"?{src:n,alt:"",description:""}:{src:n.src||"",alt:n.alt||"",description:n.description||""}),u=Array.from({length:F},(n,a)=>_[a%_.length]);let C=2654435769;const U=()=>{C=C+2654435769>>>0;let n=C;return n=Math.imul(n^n>>>16,73244475),n=Math.imul(n^n>>>16,73244475),n=n^n>>>16,(n>>>0)/4294967295};for(let n=u.length-1;n>0;n--){const a=Math.floor(U()*(n+1)),M=u[n];u[n]=u[a],u[a]=M}for(let n=1;n<u.length;n++)if(u[n].src===u[n-1].src){for(let a=n+1;a<u.length;a++)if(u[a].src!==u[n-1].src){const M=u[n];u[n]=u[a],u[a]=M;break}}return D.map((n,a)=>({...n,src:u[a].src,alt:u[a].alt,description:u[a].description}))}function Ze(f,z,A,Y,X){const D=360/X/2,F=D*(f+(A-1)/2);return{rotateX:D*(z-(Y-1)/2),rotateY:F}}function He({images:f=[],fit:z=.5,fitBasis:A="auto",minRadius:Y=600,maxRadius:X=1/0,padFactor:D=.25,overlayBlurColor:F="#030412",maxVerticalRotationDeg:_=fe.maxVerticalRotationDeg,dragSensitivity:u=fe.dragSensitivity,enlargeTransitionMs:C=fe.enlargeTransitionMs,segments:U=fe.segments,dragDampening:n=2,openedImageWidth:a="420px",openedImageHeight:M="420px",imageBorderRadius:Q="20px",openedImageBorderRadius:S="24px",grayscale:V=!1}){const P=r.useRef(null),se=r.useRef(null),ze=r.useRef(null),oe=r.useRef(null),ae=r.useRef(null),Ae=r.useRef(null),q=r.useRef(null),he=r.useRef(null),L=r.useRef(null),T=r.useRef({x:0,y:0}),ye=r.useRef({x:0,y:0}),G=r.useRef(null),J=r.useRef(!1),ce=r.useRef(!1),Z=r.useRef(!1),H=r.useRef(null),B=r.useRef("mouse"),le=r.useRef(null),W=r.useRef(!1),Ce=r.useRef(0),ge=r.useRef(0),ue=r.useRef(!1),Ee=r.useCallback(()=>{ue.current||(ue.current=!0,document.body.style.overflow="hidden",document.body.style.touchAction="none")},[]),$e=r.useCallback(()=>{var t;ue.current&&((t=P.current)==null?void 0:t.getAttribute("data-enlarging"))!=="true"&&(ue.current=!1,document.body.style.overflow="",document.body.style.touchAction="")},[]),_e=r.useMemo(()=>Ve(f,U),[f,U]),de=(t,i)=>{const h=ze.current;h&&(h.style.transform=`translateZ(calc(var(--radius) * -1)) rotateX(${t}deg) rotateY(${i}deg)`)},Te=r.useRef(null);r.useEffect(()=>{const t=P.current;if(!t)return;const i=new ResizeObserver(h=>{var c;const o=h[0].contentRect,d=Math.max(1,o.width),g=Math.max(1,o.height),m=Math.min(d,g),R=Math.max(d,g),k=d/g;let y;switch(A){case"min":y=m;break;case"max":y=R;break;case"width":y=d;break;case"height":y=g;break;default:y=k>=1.3?d:m}let p=y*z;const v=g*1.35;p=Math.min(p,v),p=re(p,Y,X),Te.current=Math.round(p);const s=Math.max(8,Math.round(m*D));t.style.setProperty("--radius",`${Te.current}px`),t.style.setProperty("--viewer-pad",`${s}px`),t.style.setProperty("--overlay-blur-color",F),t.style.setProperty("--tile-radius",Q),t.style.setProperty("--enlarge-radius",S),t.style.setProperty("--image-filter",V?"grayscale(1)":"none"),de(T.current.x,T.current.y);const w=(c=ae.current)==null?void 0:c.querySelector(".enlarge");if(w&&oe.current&&se.current){const e=oe.current.getBoundingClientRect(),E=se.current.getBoundingClientRect();if(a&&M){const b=document.createElement("div");b.style.cssText=`position: absolute; width: ${a}; height: ${M}; visibility: hidden;`,document.body.appendChild(b);const j=b.getBoundingClientRect();document.body.removeChild(b);const ee=e.left-E.left+(e.width-j.width)/2,ve=e.top-E.top+(e.height-j.height)/2;w.style.left=`${ee}px`,w.style.top=`${ve}px`}else w.style.left=`${e.left-E.left}px`,w.style.top=`${e.top-E.top}px`,w.style.width=`${e.width}px`,w.style.height=`${e.height}px`}});return i.observe(t),()=>i.disconnect()},[z,A,Y,X,D,F,V,Q,S,a,M]),r.useEffect(()=>{de(T.current.x,T.current.y)},[]);const xe=r.useCallback(()=>{H.current&&(cancelAnimationFrame(H.current),H.current=null)},[]),Pe=r.useCallback((t,i)=>{let o=re(t,-1.4,1.4)*80,d=re(i,-1.4,1.4)*80,g=0;const m=re(n??.6,0,1),R=.94+.055*m,k=.015-.01*m,y=Math.round(90+270*m),p=()=>{if(o*=R,d*=R,Math.abs(o)<k&&Math.abs(d)<k){H.current=null;return}if(++g>y){H.current=null;return}const v=re(T.current.x-d/200,-_,_),s=Ue(T.current.y+o/200);T.current={x:v,y:s},de(v,s),H.current=requestAnimationFrame(p)};xe(),H.current=requestAnimationFrame(p)},[n,_,xe]);Oe({onDragStart:({event:t})=>{var h,o;if(q.current)return;xe(),B.current=t.pointerType||"mouse",B.current==="touch"&&t.preventDefault(),B.current==="touch"&&Ee(),J.current=!0,ce.current=!1,Z.current=!1,ye.current={...T.current},G.current={x:t.clientX,y:t.clientY};const i=(o=(h=t.target).closest)==null?void 0:o.call(h,".item__image");le.current=i||null},onDrag:({event:t,last:i,velocity:h=[0,0],direction:o=[0,0],movement:d})=>{if(q.current||!J.current||!G.current)return;B.current==="touch"&&t.preventDefault();const g=t.clientX-G.current.x,m=t.clientY-G.current.y;Z.current||g*g+m*m>16&&(Z.current=!0);const R=re(ye.current.x-m/u,-_,_),k=ye.current.y+g/u,y=T.current;if((y.x!==R||y.y!==k)&&(T.current={x:R,y:k},de(R,k)),i){J.current=!1;let p=!1;if(G.current){const x=t.clientX-G.current.x,b=t.clientY-G.current.y,j=x*x+b*b,ee=B.current==="touch"?10:6;j<=ee*ee&&(p=!0)}let[v,s]=h;const[w,c]=o;let e=v*w,E=s*c;if(!p&&Math.abs(e)<.001&&Math.abs(E)<.001&&Array.isArray(d)){const[x,b]=d;e=x/u*.02,E=b/u*.02}!p&&(Math.abs(e)>.005||Math.abs(E)>.005)&&Pe(e,E),G.current=null,ce.current=!p,p&&le.current&&!q.current&&be(le.current),le.current=null,ce.current&&setTimeout(()=>ce.current=!1,120),Z.current&&(ge.current=performance.now()),Z.current=!1,B.current==="touch"&&$e()}}},{target:se,eventOptions:{passive:!1}}),r.useEffect(()=>{const t=Ae.current;if(!t)return;const i=()=>{var e,E;if(performance.now()-Ce.current<250)return;const o=q.current;if(!o)return;const d=o.parentElement,g=(e=ae.current)==null?void 0:e.querySelector(".enlarge");if(!g)return;L.current&&(L.current.remove(),L.current=null);const m=d.querySelector(".item__image--reference"),R=he.current;if(!R){g.remove(),m&&m.remove(),d.style.setProperty("--rot-y-delta","0deg"),d.style.setProperty("--rot-x-delta","0deg"),o.style.visibility="",o.style.zIndex=0,q.current=null,(E=P.current)==null||E.removeAttribute("data-enlarging"),W.current=!1;return}const k=g.getBoundingClientRect(),y=P.current.getBoundingClientRect(),p={left:R.left-y.left,top:R.top-y.top,width:R.width,height:R.height},v={left:k.left-y.left,top:k.top-y.top,width:k.width,height:k.height},s=document.createElement("div");s.className="enlarge-closing",s.style.cssText=`
        position: absolute;
        left: ${v.left}px;
        top: ${v.top}px;
        width: ${v.width}px;
